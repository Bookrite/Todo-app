...
let tasks = []
async function createTaskArrr() {
  let response = await fetch(`http://localhost:3000/tasks?username=${localStorage.username}`)
...
async function taskCompleted(btnComleteElement) {
  ...
  await fetch(`http://localhost:3000/tasks/${id}?username=${localStorage.username}`, {
    method: 'PATCH'
  })
...
async function taskDeleted(btnComleteElement) {
  let id = btnComleteElement.parentNode.parentNode.getAttribute('data-task-id');
  await fetch(`http://localhost:3000/tasks/${id}?username=${localStorage.username}`, {
    method: 'delete'
  })
 tasks = []
await createTaskArrr()
  displayTasks();
}
...
function buildTaskElement(task, index) {
  ...
  taskElement.innerHTML = `<p>${task.title}</p>
    ...
    ${task.done ? "" :
      ...
      <button onclick="taskDeleted(this)">
        <svg
          width="19"
          height="24"
          viewBox="0 0 25 24"
